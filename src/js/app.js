...
(function() {
    ...
    $$('.form-social-proof').forEach(container => {
      container.innerHTML = `
        <a href="https://g.page/r/CSHCy85xJ8VOEBM/review" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;background:var(--white);border:1px solid var(--border);border-radius:var(--border-radius);text-decoration:none;color:var(--charcoal);font-size:var(--text-sm);font-weight:500;margin-bottom:12px;">
          ...
          <span style="display:flex;align-items:center;gap:4px;"><strong style="color:var(--navy);">5.0</strong> <span style="color:#FBBC05;">&#9733;&#9733;&#9733;&#9733;&#9733;</span> <span style="color:var(--slate);">(30 reviews)</span></span>
        </a>
  ...
  function initChatWidget() {
    // Chat widget logic now lives in src/js/chat-widget.js (shared with the
    // standalone intake page). init() with no opts applies the default
    // auto-init + path suppression (/intake|/portal|/partner|/admin).
    if (window.TWAChat && window.TWAChat.init) window.TWAChat.init();
  }
