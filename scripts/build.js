...
function injectVersion(html) {
  ...
  html = html.replace(/from \d+ Google reviews/g, `from ${_reviews.count} Google reviews`);
  html = html.replace(/\(\d+ reviews\)/g, `(${_reviews.count} reviews)`);
  html = html.replace(/\(\d+ Google reviews\)/g, `(${_reviews.count} Google reviews)`);
  html = html.replace(/"reviewCount":\s*"\d+"/g, `"reviewCount": "${_reviews.count}"`);
  html = html.replace(/"ratingValue":\s*"[\d.]+"/g, `"ratingValue": "${_reviews.rating}"`);
        ...
        <div class="section-header">
          ...
          <a href="https://g.page/r/CSHCy85xJ8VOEBM/review" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:8px;margin-top:var(--space-sm);padding:8px 16px;background:var(--white);border:1px solid var(--border);border-radius:var(--border-radius);text-decoration:none;color:var(--charcoal);font-size:var(--text-sm);font-weight:500;">
            ...
            <span style="display:flex;align-items:center;gap:4px;">
              <strong style="color:var(--navy);">${_reviews.rating}</strong>
              <span style="color:#FBBC05;">&#9733;&#9733;&#9733;&#9733;&#9733;</span>
              <span style="color:var(--slate);">(${_reviews.count} reviews)</span>
            </span>
          </a>
        </div>
...
// Nav, footer, scripts, and GTM come from the shared module (single source of truth)
const { renderNav, renderFooter: _renderFooter, renderScripts, renderHead_GTM, renderBody_GTM } = require('./shared-templates');

// Wrap renderFooter to pass the office + review data automatically
      ...
      <div class="container container--narrow">
        ...
        ${(() => {
          ...
          if (cityProductConfig) {
            // City has bridge pages — show specific product links + generic hub links
...
${productCards}
          ...
          <a href="/life-health/" class="card" style="text-decoration:none;"><h3 class="card__title" style="font-size:var(--text-xl);">All Life &amp; Health</h3><p class="card__text">Health, life, disability, and employee benefits.</p><span class="card__link">View options ${arrowSvg}</span></a>
            ...
            // No bridge pages — show 3 generic hub links
            return `<div class="grid grid--3" style="margin:var(--space-xl) 0;">
...
function generateCityProductPage(cityConfig, prod) {
  ...
  // Find product data from products.json for cost/coverage/exclusion details
  const lineProducts = products[prod.line] || [];
  const productData = lineProducts.find(p => p.id === prod.product_id) || {};

  // Build parent product page link
  ...
  // Product details section (cost, exclusions) from products.json
...
// Generate city+product bridge pages
...
for (const cityConfig of cityProducts) {
  for (const prod of cityConfig.products) {
    const html = generateCityProductPage(cityConfig, prod);
